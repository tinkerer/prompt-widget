...
- **Feedback list** — paginated inbox with filters (type, status, tag, search), batch operations, quick-dispatch to agents
...
- **Agents** — configure agent endpoints (webhook or PTY-based, permission profiles, prompt templates, tool allowlists)
...
The sidebar has a resizable sessions drawer with search and quick archive for managing active agent sessions.
...
- **Tmux integration** — if tmux is available, sessions persist across service restarts. On startup, orphaned `pw-*` tmux sessions are automatically recovered.
...
- **Kill/resume** — running sessions can be killed or resumed from the admin UI
