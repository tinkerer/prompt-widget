...
export function registerAutoDispatch() {
  feedbackEvents.on('new', (event: { id: string; appId: string | null; autoDispatch?: boolean }) => {
...
async function handleAutoDispatch(event: { id: string; appId: string | null; autoDispatch?: boolean }) {
  ...
  const result = await dispatchFeedbackToAgent({ feedbackId: event.id, agentEndpointId: defaultAgent.id });
