...
function TmuxConfigManager() {
  ...
  const deleteConfig = async (id: string) => {
    if (!confirm('Delete this tmux configuration?')) return;
          ...
          <div key={cfg.id} class="tmux-config-card">
            <div class="tmux-config-header" onClick={() => setExpandedId(expanded ? null : cfg.id)}>
              ...
              <div style="display:flex;gap:4px" onClick={(e) => e.stopPropagation()}>
                ...
                {!cfg.isDefault && (
                  <button class="btn btn-sm btn-danger" onClick={() => deleteConfig(cfg.id)} style="font-size:11px;padding:2px 6px">Delete</button>
                )}
              </div>
            </div>
            {terminalSessionId && (
